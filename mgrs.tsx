...
//Type '{ top: any; right: any; bottom: number; left: number; lat?: undefined; lon?: undefined; } | { lat: number; lon: number; top?: undefined; right?: undefined; bottom?: undefined; left?: undefined; }' is not assignable to type 'LatLong'.
// Type '{ lat: number; lon: number; top?: undefined; right?: undefined; bottom?: undefined; left?: undefined; }' is not assignable to type 'LatLong'.
// Types of property 'bottom' are incompatible.
 ...
 */
export const LLtoMGRS = (ll: number[], accuracy: number = 5) => {
  const [lon, lat] = ll;
  if (lon < -180 || lon > 180) {
 ...
 */
export const mgrsToLLBounds = (mgrs: string) => {
  const bbox = UTMtoLL(decode(mgrs.toUpperCase()));
  return [bbox.left, bbox.bottom, bbox.right, bbox.top];
};

export const mgrsToPoint = (mgrs: string) => {
  if (mgrs === "") {
...
export const LLtoUTM = (ll: { lat: any; lon: any }) => {
  const Lat = ll.lat;
  const Long = ll.lon;
 ...
 * is then broken down into the ZoneNumber and ZoneLetter. exported for testing, not intended for direct use.
 *
 * @private
...
export const UTMtoLL = (utm: UTM) => {
  ...
  if (typeof utm.accuracy === "number") {
    const topRight: LatLong = UTMtoLL({
...
/**
 * Calculates the MGRS letter designator for the given latitude. exported for testing
 *
 * @param {number} latitude The latitude in WGS84 to get the letter designator
 *     for.
 * @return {string} The letter designator.
 */
export const getLetterDesignator = (latitude: number) => {
 ...
 */
export const encode = (utm: UTM, accuracy: number | undefined) => {
  // prepend with leading zeroes
  const seasting = "00000" + utm.easting,
    snorthing = "00000" + utm.northing;

  return `${utm.zoneNumber + utm.zoneLetter!} ${get100kID(utm.easting, utm.northing, utm.zoneNumber)} ${seasting.substr(
    seasting.length - 5,
    accuracy
 ...
 */
const get100kID = (easting: number, northing: number, zoneNumber: number) => {
...
/**
 * Given a UTM zone number, figure out the MGRS 100K set it is in. Exported for testing
 *
 * @private
 * @param {number} i An UTM zone number.
 * @return {number} the 100k set the UTM zone is in.
 */
export const get100kSetForZone = (i: number) => {
 ...
 * translated from the UTM northing, easting and zone number. Exported for testing
 *
 * @private
 ...
 */
export const getLetter100kID = (column: number, row: number, parm: number) => {
...
export const decode = (mgrsString: string) => {
  for (
    var testChar, length = mgrsString.length, hunK = null, sb = "", i = 0;
    !/[A-Z]/.test((testChar = mgrsString.charAt(i)));

  ) {
    if (i >= 2) throw new Error("MGRSPoint bad conversion from: " + mgrsString);
    sb += testChar;
    i++;
  }
  var zoneNumber = parseInt(sb, 10);
  if (0 === i || i + 3 > length) throw new Error("MGRSPoint bad length from: " + mgrsString);
 ...
 * should be added to the other, secondary easting value. Exported for testing
 *
 * @private
 ...
 */
export const getEastingFromChar = (e: string, set: number) => {
 ...
 * to be added for the zone letter of the MGRS coordinate. Exported for testing
 *
 * @private
 ...
 */
export const getNorthingFromChar = (n: string, set: number) => {
  if (n > "V") {
...
/**
 * The const getMinNorthing returns the minimum northing value of a MGRS
 * zone. Exported for testing
 *
 ...
 * @return {number}
 */
export const getMinNorthing = (zoneLetter: string) => {
  let northing;
  switch (zoneLetter) {
    case "C":
      northing = 1100000;
